...
typedef struct _stack {
	char *stack;		/*(문자열) stack으로 사용되는 동적할당 배열을 가리키는 포인터 변수 */
	int size;		/* 스택의 크기(size) */
	int top;		/* 스택의 입,출구 위치정보 저장 */
}Stack;

typedef struct _intstack {
	int *istack;      /*(정수) stack으로 사용되는 동적할당 배열을 가리키는 포인터 변수 */
	int size;       /* 스택의 크기(size) */
	int top;        /* 스택의 입,출구 위치정보 저장 */
}IStack;

/*함수 선언부*/
int createStack(Stack *sp, int size);    /*문자 스택 메모리의 할당*/
int isStackFull(Stack *sp);              /*문자 스택이 꽉 차있는지 검사*/
int isStackEmpty(Stack *sp);             /*문자 스택이 비어있는지 검사*/
int push(Stack *sp, char inData);        /*문자 스택에 데이터를 하나 저장함*/
int pop(Stack *sp, char *popData);       /*문자 스택에서 데이터를 꺼냄*/
char peek(Stack *sp);                    /*문자 스택에서 맨 위의 데이터를 봄*/
int createIStack(IStack *sp, int size);  /*정수 스택 메모리의 할당*/
int isIStackFull(IStack *sp);            /*정수 스택이 꽉 차있는지 검사*/
int isIStackEmpty(IStack *sp);           /*정수 스택이 비어있는지 검사*/
int ipush(IStack *sp, int inData);       /*정수 스택에 데이터를 하나 저장함*/
int ipop(IStack *sp, int *popData);      /*정수 스택에서 데이터를 꺼냄*/
int priOperator(char op);                /*연산자의 우선순위를 정해주는 함수*/
int comOperator(char op1, char op2);     /*연산자의 우선순위를 비교해주는 함수*/
void newArray(char str[]);               /*띄어쓰기가 입력된 수식을 새롭게 하나의 배열에 넣는 작업을 해주는 함수*/
int calc(int num1, int num2, char c);    /*연산을 수행하는 함수*/
void convAndCalc(char str[]);            /*중위 표기식을 후위 표기식으로 변환, 후위 표기식을 이용하여 연산하는 함수*/
